...
const getAvailableCountries = async (req, res, next) =>{
    try {
        ...
        if (!response.ok) throw new HttpError("Sorry, we could not fetch the API. This likely means that the server is down. Please try again later.", 503);
        ...
        return res.status(500).json({error: 'Sorry, an unexpected error occurred while trying to reach the API. Please try again later.'})
    }
}
...
const getCountryInfo = async (req, res, next) => {
    try {
        ...
        const borderResponse = await fetch(`https://date.nager.at/api/v3/CountryInfo/${code}`);
        if (!borderResponse.ok) throw new HttpError('An unnexpected error occurred while trying to reach the Nager.Date API.', 500);
        let borderData = await borderResponse.json();
        responseObj.borders = borderData.borders;
        ...
        if (!populationResponse.ok) throw new HttpError('An unnexpected error occurred while trying to get the population count.', 500);
        ...
        if (!flagResponse.ok) throw new HttpError('An unnexpected error occurred while trying to get the flag data.', 500);
